<template>
    <div :class="{'ov-h':!awakenApp}" class="box" style="height:100vh;overflow: hidden;">
    <!-- <div :class="{'ov-h':!awakenApp}" class="box" style="overflow: scroll;height: auto;min-height:100vh;overflow-x: hidden;" v-show="isShowAll"> -->
        <animate-get-money v-if="getMoney"></animate-get-money>
		<div ref="wrapper" class="wrapper" style="width:100%;height:100vh;overflow-y:scroll;overflow-x:hidden;">
			<div class="content">
				...
				<!-- 标签 -->
				<activity-tags :labels="result.activityLabel" :type='3'></activity-tags>
				
				<!-- 用户在app内打开--显示底部按钮，3rem的高度撑开 -->
				<div style="height:3rem;" v-if="userType!=1 && !awakenApp"></div>
				<!-- 否则34px -->
				<div style="height:34px;" v-else></div>
				
				<awaken-app v-if="awakenApp&&result.kid" :result="result" :type='2'></awaken-app>
			</div>
		...
		<!-- 底部按钮-->
		<activity-bottom-btn v-if="userType!=1" @sureQuestionBtn="sureQuestionBtn" :result="result"></activity-bottom-btn>
    </div>
</template>
<style lang="less">
.content{
...
export default {
	...
	data() {
		return {
			...
			videoMark: videoMark,
			isShowAll: false,
			isChoselabelShow:false,//是否显示标签选择
	...
	created() {
		...
		this.$nextTick(function() {
			...
			http.activityDetail(this.activityId, function(response) {
				...
				switch(self.result.activityForm){
                    case 0:
                        document.title = '线下活动'
                        break;
                    case 1:
